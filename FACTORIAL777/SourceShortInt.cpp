...
vector <short int> Factorial(vector <short int> n) {  //Факториал через рекурсию. Работает на порядки!!! быстрее обычного цикла.

	vector <short int> One = { 1 };
	size_t null = 0;

	for (size_t i = 0; i < n.size(); ++i) { if (n [i] == 0) ++null;}   // Проверка всех элементов вектора на равенство нулю
	
	if (null == n.size()) return One;      // Конец рекурсии при достижении нуля
...
vector <short int> Factorial_easy (int n) { //Факториал через цикл
