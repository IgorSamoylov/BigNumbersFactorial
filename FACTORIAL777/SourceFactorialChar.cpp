...
vector <char> operator + (const vector <char> &v1, const vector <char> &v2) // Функция перегружает штатный оператор сложения для использования с векторами
{
...
vector <char> operator * (const vector <char> &x, const vector <char> &y)     // Функция перегружает штатный оператор умножения для использования с векторами
{
...
vector <char> operator - (const vector <char> &v1, const vector <char> &v2) { // Перегруженный оператор вычитания для поразрядных векторов чисел
...
size_t Logic = 0;	                            // Поиск отрицательных значений и последовательный перенос единиц из старших разрядов
	while (Logic < result.size() - 1) {
		for (size_t i = 0; i < result.size(); ++i) {
			if (result[i] < 0) {
...
vector <char> Vectorizator(int n) {    //Функция преобразует входящее натуральное число в вектор по разрядам
...
vector <char> Factorial(vector <char> n) {  //Факториал через рекурсию. Работает на порядки!!! быстрее обычного цикла.
...
vector <char> Factorial_easy (int n) { //Факториал через цикл
